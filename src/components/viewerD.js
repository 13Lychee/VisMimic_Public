...
export class ViewerD {
		...
		return new Promise((resolve, reject) => {
			...
			loader.load(
				...
				(gltf) => {
					...
					console.log("track data:", clips[0].tracks);
	...
	setContent(object, clips) {
		...
		this.defaultCamera.position.set(-1, 1, 7);
		...
		if (this.options.cameraPosition) {
			...
			const center = new Vector3(-1, 20, 0);
...
export class DataParser {
	...
	load(url) {
		return new Promise((resolve, reject) => {
			const loader = new GLTFLoader(MANAGER)
				.setCrossOrigin('anonymous')
				.setDRACOLoader(DRACO_LOADER)
				.setKTX2Loader(KTX2_LOADER.detectSupport(window.renderer))
				.setMeshoptDecoder(MeshoptDecoder);
	...
	getFrameData() {
		// const frameData = this.clips[0].tracks[0].times;
		const frameCount = this.clips[0].tracks[0].times.length;
		const frameData = Array.from({ length: frameCount }, (_, index) => index);
		// 结果: [0, 1, 2, ..., 167]
		// const frameData = [...Array(frameCount).keys()];
		// 结果: [0, 1, 2, ..., 167]
